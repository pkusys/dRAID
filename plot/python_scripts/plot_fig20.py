...
def collect_data(draid_path, spdk_path, linux_path):
    global spdk_iops, draid_iops, spdk_lat, draid_lat
    for i in filenames:
        extracted_data = parse_log(os.path.join(draid_path, i))
        if 'iops' in extracted_data:
            draid_iops.append(extracted_data['iops'])
        else:
            draid_iops.append(0)
        if 'lat' in extracted_data:
            draid_lat.append(extracted_data['lat'])
        else:
            draid_lat.append(0)
        extracted_data = parse_log(os.path.join(spdk_path, i))
        if 'iops' in extracted_data:
            spdk_iops.append(extracted_data['iops'])
        else:
            spdk_iops.append(0)
        if 'lat' in extracted_data:
            spdk_lat.append(extracted_data['lat'])
        else:
            spdk_lat.append(0)
    draid_iops = np.array(draid_iops)
    spdk_iops = np.array(spdk_iops)
...
if not os.path.exists('plots'):
    os.makedirs('plots')
_raid5_app(spdk_iops,draid_iops,'plots/fig20a.pdf')
_raid5_app(spdk_lat,draid_lat,'plots/fig20b.pdf')
