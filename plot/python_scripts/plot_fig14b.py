...
draid_lat_rw = []
draid_bw_rw = []
draid_filenames = ['2.log','4.log','8.log','12.log','16.log','24.log','32.log','48.log','64.log','96.log','128.log','144.log']
spdk_filenames = ['4.log','8.log','12.log','16.log','20.log','24.log','30.log']
linux_filenames = ['1.log','2.log','3.log','4.log','5.log','6.log','7.log']
...
def collect_data(draid_path, spdk_path, linux_path):
    global linux_bw_read, linux_bw_write, spdk_bw_read, spdk_bw_write, draid_bw_read, draid_bw_write, linux_lat_read, \
        linux_lat_write, spdk_lat_read, spdk_lat_write, draid_lat_read, draid_lat_write, linux_lat_rw, linux_bw_rw, \
        spdk_lat_rw, spdk_bw_rw, draid_lat_rw, draid_bw_rw, draid_filenames, spdk_filenames, linux_filenames
    ...
    linux_lat_rw = (linux_lat_write + linux_lat_read) / 2
    linux_bw_rw = linux_bw_write + linux_bw_read
    spdk_lat_rw = (spdk_lat_write + spdk_lat_read) / 2
    spdk_bw_rw = spdk_bw_write + spdk_bw_read
    draid_lat_rw = (draid_lat_write + draid_lat_read) / 2
    draid_bw_rw = draid_bw_write + draid_bw_read
...
def _raid5_readwrite():
    ...
    ax.plot(linux_bw_rw, linux_lat_rw, linewidth=4, color='tab:blue', linestyle='solid', label='Linux', marker="o", markersize=ms, alpha=0.8)

    ax.plot(spdk_bw_rw, spdk_lat_rw, linewidth=4, color='tab:orange', linestyle='solid', label='SPDK', marker="^", markersize=ms, alpha=0.8)

    ax.plot(draid_bw_rw, draid_lat_rw, linewidth=4, color='tab:green', linestyle='solid', label='dRAID', marker="s", markersize=ms, alpha=0.8)

    plt.axvline(x=11.4 * 1024 * 2, linewidth=3, color='tab:red', label='NIC Goodput')
    ...
    plt.xticks(ticks=[0,5000,10000,15000,20000], labels=[0,5000,10000,15000,20000])
    plt.savefig('plots/fig14b.pdf', bbox_inches='tight', pad_inches=0.2)
