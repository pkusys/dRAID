...
then
  ...
  sleep 3
  sudo LD_PRELOAD=../spdk_bdev /usr/local/bin/fio ../raid6_d.conf -ioengine=../spdk_bdev -iodepth=6 -rw=randrw -rwmixread=100 -bs=128k -numjobs=2
  exit 0
fi
...
then
  ...
  sleep 4
  sudo LD_PRELOAD=../spdk_bdev /usr/local/bin/fio ../raid6_d.conf -ioengine=../spdk_bdev -iodepth=12 -rw=randrw -rwmixread=100 -bs=128k -numjobs=2
  exit 0
fi
...
then
  ...
  sleep 5
  sudo LD_PRELOAD=../spdk_bdev /usr/local/bin/fio ../raid6_d.conf -ioengine=../spdk_bdev -iodepth=13 -rw=randrw -rwmixread=100 -bs=128k -numjobs=2
  exit 0
fi
...
then
  ...
  sleep 9
  sudo LD_PRELOAD=../spdk_bdev /usr/local/bin/fio ../raid6_d.conf -ioengine=../spdk_bdev -iodepth=14 -rw=randrw -rwmixread=100 -bs=128k -numjobs=2
  exit 0
fi
