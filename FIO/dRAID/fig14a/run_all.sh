...
do
  ...
  then
    ...
    sleep 10
    sudo timeout -k 5 200 LD_PRELOAD=../spdk_bdev /usr/local/bin/fio ../raid5.conf -ioengine=../spdk_bdev -iodepth=$i -rw=randwrite -bs=128k -numjobs=2 > "results/${total_io_depth}.log"
  fi
done
