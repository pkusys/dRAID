...
then
  ...
  sleep 10
  sudo LD_PRELOAD=../spdk_bdev /usr/local/bin/fio ../raid5.conf -ioengine=../spdk_bdev -iodepth=$io_depth_per_job -rw=randrw -rwmixread=50 -bs=128k -numjobs=2 > results/${io_depth}.log
  exit 0
fi
