...
then
  ...
  sleep 3
  sudo LD_PRELOAD=../spdk_bdev /usr/local/bin/fio ../raid5_d.conf -ioengine=../spdk_bdev -iodepth=8 -rw=randrw -rwmixread=100 -bs=128k -numjobs=2
  exit 0
fi
...
then
  ...
  sleep 4
  sudo LD_PRELOAD=../spdk_bdev /usr/local/bin/fio ../raid5_d.conf -ioengine=../spdk_bdev -iodepth=12 -rw=randrw -rwmixread=100 -bs=128k -numjobs=2
  exit 0
fi
...
then
  ...
  sleep 6
  sudo LD_PRELOAD=../spdk_bdev /usr/local/bin/fio ../raid5_d.conf -ioengine=../spdk_bdev -iodepth=17 -rw=randrw -rwmixread=100 -bs=128k -numjobs=2
  exit 0
fi
