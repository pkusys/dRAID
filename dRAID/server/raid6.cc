...
static unsigned char gf_const_g[255];
static unsigned char gf_const_tbl_arr[255][32];
static unsigned char gf_const_tbl_arr_p[255][255][32];
static unsigned char gf_const_tbl_arr_q[255][255][32];
static unsigned char gf_const_tbl_arr_a[255][32];
static unsigned char gf_const_tbl_arr_b[255][255][32];
...
static uint8_t
data_index_dis(uint8_t a, uint8_t b)
{
    return a < b ? 255u - (b - a) : a - b;
}
...
is_rc(uint8_t type)
{
    return type >= RECON_NRT && type <= RECON_RT_DD;
}

bool
need_read(uint8_t type)
{
    return type == RECON_RT || type == RECON_RT_DD || type == RECON_RT_DP;
}

bool
need_p_only(uint8_t type)
{
    return type == RECON_RT || type == RECON_NRT;
}

bool
need_q_only(uint8_t type)
{
    return type == RECON_RT_DP || type == RECON_NRT_DP || type == RECON_NRT_DP_Q;
}

bool
need_p(uint8_t type)
{
    return is_rc(type) && !need_q_only(type);
}

bool
need_q(uint8_t type)
{
    return is_rc(type) && !need_p_only(type);
}

bool
need_pq(uint8_t type)
{
    return type == RECON_NRT_DD || type == RECON_NRT_DD_P || type == RECON_NRT_DD_Q || type == RECON_RT_DD;
}

bool
is_q_while_need_q(uint8_t type)
{
    return type == RECON_NRT_DP_Q || type == RECON_NRT_DD_Q;
}

bool
is_p_while_need_q(uint8_t type)
{
    return type == RECON_NRT_DD_P;
}

bool
is_data_while_need_q(uint8_t type)
{
    return type == RECON_NRT_DP || type == RECON_RT_DP || type == RECON_NRT_DD || type == RECON_RT_DD;
}
    ...
    switch (recv_cs_msg->type) {
        ...
        case FWD_MIX:
            ...
            if (tgt == PEER1) {
                ...
                gf_vect_mul(cs_msg->length << bdev_context->blk_size_shift, gf_const_tbl_arr[recv_cs_msg->data_index],
                            bdev_context->buff1 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                            bdev_context->buff3 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift));
                cs_msg->sgl[0].addr = (uint64_t) bdev_context->buff3 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift);
            }
        ...
        case RECON_NRT_DD_Q:
        case RECON_RT_DD:
            size = sizeof(cs_message_t) - sizeof(struct sg_entry) * 30;
            cs_msg->type = SD_RC;
            cs_msg->length = recv_cs_msg->fwd_length;
            cs_msg->offset = recv_cs_msg->fwd_offset;
            cs_msg->last_index = g_process_id;
            ...
            mr = (struct ibv_mr *) spdk_mem_map_translate(rqp->mem_map, cs_msg->sgl[0].addr, NULL);
            cs_msg->sgl[0].rkey = mr->rkey;
            cs_msg->sgl[1].addr = (uint64_t) bdev_context->buff3 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift);
            cs_msg->sgl[1].len = (uint32_t) cs_msg->length << bdev_context->blk_size_shift;
            mr = (struct ibv_mr *) spdk_mem_map_translate(rqp->mem_map, cs_msg->sgl[1].addr, NULL);
            cs_msg->sgl[1].rkey = mr->rkey;
            cs_msg->req_id = (uint64_t) send_wrapper;
            sge->length = size;
            send_wr->imm_data = kReqTypePeer;
            send_wr->send_flags = 0;
        ...
        case FWD_RO_PRO:
            size = sizeof(cs_message_t) - sizeof(struct sg_entry) * 31;
            cs_msg->type = SD_DF;
            cs_msg->length = recv_cs_msg->length;
            cs_msg->offset = recv_cs_msg->offset;
            cs_msg->last_index = g_process_id;
            cs_msg->num_sge[0] = 1;
            cs_msg->num_sge[1] = 0;
            if (tgt == PEER1) {
                std::memset(bdev_context->buff1 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                            0, cs_msg->length << bdev_context->blk_size_shift);
                // d_chunk_x
                if (recv_cs_msg->fwd_length > 0) {
                    gf_const_tbl = gf_const_tbl_arr_p[data_index_dis(recv_cs_msg->x_index, recv_cs_msg->y_index)]
                            [data_index_dis(recv_cs_msg->data_index, recv_cs_msg->y_index)];
                    gf_vect_mul(recv_cs_msg->fwd_length << bdev_context->blk_size_shift, gf_const_tbl,
                                bdev_context->buff2 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                bdev_context->buff3 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift));
                    raid5_xor_buf(bdev_context->buff1 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  bdev_context->buff3 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_cs_msg->fwd_length << bdev_context->blk_size_shift);
                }
                // d_chunk_y
                if (recv_cs_msg->y_length > 0) {
                    gf_const_tbl = gf_const_tbl_arr_p[data_index_dis(recv_cs_msg->y_index, recv_cs_msg->x_index)]
                            [data_index_dis(recv_cs_msg->data_index, recv_cs_msg->x_index)];
                    gf_vect_mul(recv_cs_msg->y_length << bdev_context->blk_size_shift, gf_const_tbl,
                                bdev_context->buff2 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                                bdev_context->buff3 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift));
                    raid5_xor_buf(bdev_context->buff1 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  bdev_context->buff3 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_cs_msg->y_length << bdev_context->blk_size_shift);
                }
                cs_msg->sgl[0].addr = (uint64_t) bdev_context->buff1 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift);
            } else {
                std::memset(bdev_context->buff4 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                            0, cs_msg->length << bdev_context->blk_size_shift);
                // d_chunk_x
                if (recv_cs_msg->fwd_length > 0) {
                    gf_const_tbl = gf_const_tbl_arr_q[data_index_dis(recv_cs_msg->x_index, recv_cs_msg->y_index)]
                            [data_index_dis(recv_cs_msg->x_index, recv_cs_msg->data_index)];
                    gf_vect_mul(recv_cs_msg->fwd_length << bdev_context->blk_size_shift, gf_const_tbl,
                                bdev_context->buff2 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                bdev_context->buff3 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift));
                    raid5_xor_buf(bdev_context->buff4 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  bdev_context->buff3 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_cs_msg->fwd_length << bdev_context->blk_size_shift);
                }
                // d_chunk_y
                if (recv_cs_msg->y_length > 0) {
                    gf_const_tbl = gf_const_tbl_arr_q[data_index_dis(recv_cs_msg->y_index, recv_cs_msg->x_index)]
                            [data_index_dis(recv_cs_msg->y_index, recv_cs_msg->data_index)];
                    gf_vect_mul(recv_cs_msg->y_length << bdev_context->blk_size_shift, gf_const_tbl,
                                bdev_context->buff2 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                                bdev_context->buff3 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift));
                    raid5_xor_buf(bdev_context->buff4 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  bdev_context->buff3 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_cs_msg->y_length << bdev_context->blk_size_shift);
                }
                gf_vect_mul(cs_msg->length << bdev_context->blk_size_shift, gf_const_tbl_arr[recv_cs_msg->data_index],
                            bdev_context->buff4 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                            bdev_context->buff3 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift));
                cs_msg->sgl[0].addr = (uint64_t) bdev_context->buff3 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift);
            }
            cs_msg->sgl[0].len = (uint32_t) cs_msg->length << bdev_context->blk_size_shift;
        ...
        case FWD_RW_PRO:
            size = sizeof(cs_message_t) - sizeof(struct sg_entry) * 31;
            cs_msg->type = SD_DF;
            cs_msg->length = g_strip_size;
            cs_msg->offset = recv_cs_msg->offset - (recv_cs_msg->offset % g_strip_size);
            cs_msg->last_index = g_process_id;
            cs_msg->num_sge[0] = 1;
            cs_msg->num_sge[1] = 0;
            if (tgt == PEER1) {
                ...
                // diff
                if (recv_cs_msg->length > 0) {
                    raid5_xor_buf(bdev_context->buff1 + ((recv_cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                                  bdev_context->buff2 + ((recv_cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_cs_msg->length << bdev_context->blk_size_shift);
                    raid5_xor_buf(bdev_context->buff1 + ((recv_cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                                  bdev_context->buff4 + ((recv_cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_cs_msg->length << bdev_context->blk_size_shift);
                }
                cs_msg->sgl[0].addr = (uint64_t) bdev_context->buff1 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift);
            } else {
                std::memset(bdev_context->buff3 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                            0, cs_msg->length << bdev_context->blk_size_shift);
                // d_chunk_x
                if (recv_cs_msg->fwd_length > 0) {
                    gf_const_tbl = gf_const_tbl_arr_q[data_index_dis(recv_cs_msg->x_index, recv_cs_msg->y_index)]
                    ...
                    gf_vect_mul(recv_cs_msg->fwd_length << bdev_context->blk_size_shift, gf_const_tbl,
                                bdev_context->buff2 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                g_bdev_context->buff1 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift));
                    raid5_xor_buf(bdev_context->buff3 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  g_bdev_context->buff1 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_cs_msg->fwd_length << bdev_context->blk_size_shift);
                }
                // d_chunk_y
                if (recv_cs_msg->y_length > 0) {
                    ...
                    gf_vect_mul(recv_cs_msg->y_length << bdev_context->blk_size_shift, gf_const_tbl,
                                bdev_context->buff2 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                                g_bdev_context->buff1 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift));
                    raid5_xor_buf(bdev_context->buff3 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  g_bdev_context->buff1 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_cs_msg->y_length << bdev_context->blk_size_shift);
                }
                // diff
                if (recv_cs_msg->length > 0) {
                    raid5_xor_buf(bdev_context->buff3 + ((recv_cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                                  bdev_context->buff2 + ((recv_cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_cs_msg->length << bdev_context->blk_size_shift);
                    raid5_xor_buf(bdev_context->buff3 + ((recv_cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                                  bdev_context->buff4 + ((recv_cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
        ...
        case PR_PRO:
            ...
            // d_chunk_x
            if (recv_cs_msg->fwd_length > 0) {
                gf_const_tbl = gf_const_tbl_arr_b[data_index_dis(recv_cs_msg->x_index, recv_cs_msg->y_index)]
                        [recv_cs_msg->x_index];
                gf_vect_mul(recv_cs_msg->fwd_length << bdev_context->blk_size_shift, gf_const_tbl,
                            bdev_context->buff2 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                            bdev_context->buff3 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift));
            ...
            // d_chunk_y
            if (recv_cs_msg->y_length > 0) {
                gf_const_tbl = gf_const_tbl_arr_b[data_index_dis(recv_cs_msg->y_index, recv_cs_msg->x_index)]
                        [recv_cs_msg->y_index];
                gf_vect_mul(recv_cs_msg->y_length << bdev_context->blk_size_shift, gf_const_tbl,
                            bdev_context->buff2 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                            bdev_context->buff3 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift));
            ...
            cs_msg->sgl[0].addr= (uint64_t) bdev_context->buff4 + ((cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift);
        ...
        case PR_QRO:
            ...
            if (recv_cs_msg->fwd_length > 0) {
                gf_const_tbl = gf_const_tbl_arr_b[data_index_dis(recv_cs_msg->x_index, recv_cs_msg->y_index)]
                        [data_index_dis(0u, recv_cs_msg->y_index)];
                gf_vect_mul(recv_cs_msg->fwd_length << bdev_context->blk_size_shift, gf_const_tbl,
                            bdev_context->buff2 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                            bdev_context->buff3 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift));
                raid5_xor_buf(bdev_context->buff4 + ((recv_cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
            ...
            if (recv_cs_msg->y_length > 0) {
                gf_const_tbl = gf_const_tbl_arr_b[data_index_dis(recv_cs_msg->y_index, recv_cs_msg->x_index)]
                        [data_index_dis(0u, recv_cs_msg->x_index)];
                gf_vect_mul(recv_cs_msg->y_length << bdev_context->blk_size_shift, gf_const_tbl,
                            bdev_context->buff2 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                            bdev_context->buff3 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift));
                raid5_xor_buf(bdev_context->buff4 + ((recv_cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
...
void
map_ibv_send_wr_one2many(struct ibv_send_wr *send_wrs, struct sg_entry *sges, uint8_t sge_cnt,
...
void
map_ibv_send_wr_two2one(struct ibv_send_wr *send_wr, struct sg_entry *sge, uint64_t addr1, uint64_t len1,
                        uint64_t addr2, uint64_t len2, struct rdma_qp_server *rqp, bool signal)
{
    struct ibv_mr *mr1 = (struct ibv_mr *) spdk_mem_map_translate(rqp->mem_map, addr1, NULL);
    struct ibv_mr *mr2 = (struct ibv_mr *) spdk_mem_map_translate(rqp->mem_map, addr2, NULL);

    struct ibv_sge *sges = send_wr->sg_list;
    send_wr->num_sge = 2;
    sges[0].addr = addr1;
    sges[0].length = len1;
    sges[0].lkey = mr1->lkey;
    sges[1].addr = addr2;
    sges[1].length = len2;
    sges[1].lkey = mr2->lkey;
    send_wr->wr.rdma.remote_addr = sge->addr;
    send_wr->wr.rdma.rkey = sge->rkey;
...
run_state_machine(struct recv_wr_wrapper_server *recv_wrapper, enum action act)
{
    ...
    switch (req_state) {
        ...
        case READ_IO_DONE:
            ...
            map_ibv_send_wr_one2many(send_wrapper->send_wr_write, recv_wrapper->cs_msg->sgl, recv_wrapper->cs_msg->num_sge[0],
        ...
        case WRITE_START:
            ...
            rqp = recv_wrapper->rqp;
            map_ibv_send_wr_one2many(recv_wrapper->send_wr_read, recv_wrapper->cs_msg->sgl, recv_wrapper->cs_msg->num_sge[0],
                                     (uint64_t) bdev_context->buff1 + ((bdev_context->offset % g_strip_size) << bdev_context->blk_size_shift),
        ...
        case FW_RO_START:
            ...
            break;
        case FW_RO_IO_DONE:
            bdev_context->state = FW_RO_PEND1;
        ...
        case FW_RW_PEND2:
            ...
            break;
        case FW_RW_PEND3:
            bdev_context->state = FW_RW_PEND4;
            if (act == IO_COMPLETE) {
                rqp = recv_wrapper->rqp;
                send_wrapper = TAILQ_FIRST(&rqp->free_send);
                if (spdk_unlikely(send_wrapper == NULL)) {
        ...
        case FW_MIX_PEND2:
            ...
            break;
        case FW_MIX_PEND3:
            bdev_context->state = FW_MIX_PEND4;
            if (act == IO_COMPLETE) {
                rqp = recv_wrapper->rqp;
                send_wrapper = TAILQ_FIRST(&rqp->free_send);
                if (spdk_unlikely(send_wrapper == NULL)) {
        ...
        case FW_MIX_PEND4:
            ...
            break;
        case FW_RO_PRO_START:
            bdev_context->state = FW_RO_PRO_IO_DONE;
            bdev_context->buff = bdev_context->buff2;
            bdev_read((void*) recv_wrapper);
            break;
        case FW_RO_PRO_IO_DONE:
            bdev_context->state = FW_RO_PRO_PEND1;
            ...
            rdma_send_msg(send_wrapper, recv_wrapper, PEER2);
            break;
        case FW_RO_PRO_PEND1:
            bdev_context->state = RELEASE;
            break;
        case FW_RW_PRO_START:
            bdev_context->state = FW_RW_PRO_PEND1;
            bdev_context->buff = bdev_context->buff2;
            bdev_read((void *) recv_wrapper);
            rqp = recv_wrapper->rqp;
            map_ibv_send_wr_one2many(recv_wrapper->send_wr_read, recv_wrapper->cs_msg->sgl, recv_wrapper->cs_msg->num_sge[0],
                                     (uint64_t) bdev_context->buff4 + ((recv_wrapper->cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                                     recv_wrapper->cs_msg->length << bdev_context->blk_size_shift, rqp, true);
            rdma_read(recv_wrapper, recv_wrapper->cs_msg->num_sge[0]);
            break;
        case FW_RW_PRO_PEND1:
            bdev_context->state = FW_RW_PRO_READ_IO1_DONE;
            break;
        case FW_RW_PRO_READ_IO1_DONE:
            bdev_context->state = FW_RW_PRO_PEND2;

            bdev_context->buff = bdev_context->buff4;
            bdev_context->offset = recv_wrapper->cs_msg->offset;
            bdev_context->size = recv_wrapper->cs_msg->length;
            ...
            bdev_write((void*) recv_wrapper);
            break;
        case FW_RW_PRO_PEND2:
            bdev_context->state = FW_RW_PRO_PEND3;
            if (act == IO_COMPLETE) {
                rqp = recv_wrapper->rqp;
                send_wrapper = TAILQ_FIRST(&rqp->free_send);
                if (spdk_unlikely(send_wrapper == NULL)) {
            ...
            break;
        case FW_RW_PRO_PEND3:
            bdev_context->state = FW_RW_PRO_PEND4;
            if (act == IO_COMPLETE) {
                rqp = recv_wrapper->rqp;
                send_wrapper = TAILQ_FIRST(&rqp->free_send);
                if (spdk_unlikely(send_wrapper == NULL)) {
        ...
        case PR_MIX_START:
            ...
            break;
        case PR_MIX_PEND1:
            bdev_context->state = PR_MIX_READ_IO1_DONE;
            break;
        case PR_MIX_READ_IO1_DONE:
            bdev_context->state = PR_MIX_IO2_DONE;
            std::memcpy(bdev_context->buff1 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                        bdev_context->buff2 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
        ...
        case PR_MIX_IO2_DONE:
            ...
            break;
        case PR_PRO_START:
            bdev_context->state = PR_PRO_PEND1;
            bdev_context->buff = bdev_context->buff2;
            bdev_context->offset = recv_wrapper->cs_msg->offset;
            bdev_context->size = recv_wrapper->cs_msg->length;
            bdev_read((void*) recv_wrapper);
            ...
            break;
        case PR_PRO_PEND1:
            bdev_context->state = PR_PRO_READ_IO1_DONE;
            break;
        case PR_PRO_READ_IO1_DONE:
            bdev_context->state = PR_PRO_PEND2;
            ...
            // d_chunk_x
            if (recv_wrapper->cs_msg->fwd_length > 0) {
                gf_const_tbl = gf_const_tbl_arr_a[data_index_dis(recv_wrapper->cs_msg->y_index, recv_wrapper->cs_msg->x_index)];
                gf_vect_mul(recv_wrapper->cs_msg->fwd_length << bdev_context->blk_size_shift, gf_const_tbl,
                            bdev_context->buff2 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                            bdev_context->buff3 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift));
                raid5_xor_buf(bdev_context->buff1 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                              bdev_context->buff3 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                              recv_wrapper->cs_msg->fwd_length << bdev_context->blk_size_shift);
            }
            // d_chunk_y
            if (recv_wrapper->cs_msg->y_length > 0) {
                gf_const_tbl = gf_const_tbl_arr_a[data_index_dis(recv_wrapper->cs_msg->x_index, recv_wrapper->cs_msg->y_index)];
                gf_vect_mul(recv_wrapper->cs_msg->y_length << bdev_context->blk_size_shift, gf_const_tbl,
                            bdev_context->buff2 + ((recv_wrapper->cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                            bdev_context->buff3 + ((recv_wrapper->cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift));
                raid5_xor_buf(bdev_context->buff1 + ((recv_wrapper->cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                              bdev_context->buff3 + ((recv_wrapper->cs_msg->y_offset % g_strip_size) << bdev_context->blk_size_shift),
                              recv_wrapper->cs_msg->y_length << bdev_context->blk_size_shift);
            }
            // new
            raid5_xor_buf(bdev_context->buff1 + ((recv_wrapper->cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                          bdev_context->buff4 + ((recv_wrapper->cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                          recv_wrapper->cs_msg->length << bdev_context->blk_size_shift);
            raid5_xor_buf(bdev_context->buff1 + ((recv_wrapper->cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                          bdev_context->buff2 + ((recv_wrapper->cs_msg->offset % g_strip_size) << bdev_context->blk_size_shift),
                          recv_wrapper->cs_msg->length<<bdev_context->blk_size_shift);
            ...
            break;
        case PR_PRO_PEND2:
            bdev_context->state = PR_PRO_PEND3;
            if (act == IO_COMPLETE) {
                rqp = recv_wrapper->rqp;
                send_wrapper = TAILQ_FIRST(&rqp->free_send);
                if (spdk_unlikely(send_wrapper == NULL)) {
        ...
        case PR_PRO_PEND3:
            ...
            break;
        case PR_QRO_START:
            bdev_context->state = PR_QRO_PEND1;
            bdev_context->buff = bdev_context->buff2;
            bdev_context->offset = recv_wrapper->cs_msg->offset;
            bdev_context->size = recv_wrapper->cs_msg->length;
            bdev_read((void*) recv_wrapper);
            ...
            break;
        case PR_QRO_PEND1:
            bdev_context->state = PR_QRO_READ_IO1_DONE;
            break;
        case PR_QRO_READ_IO1_DONE:
            bdev_context->state = PR_QRO_PEND2;
            std::memset(bdev_context->buff1 + ((bdev_context->offset % g_strip_size) << bdev_context->blk_size_shift),
                        0, bdev_context->size << bdev_context->blk_size_shift);
            // d_chunk_x
            if (recv_wrapper->cs_msg->fwd_length > 0) {
            ...
            break;
        case PR_QRO_PEND2:
            bdev_context->state = PR_QRO_PEND3;
            if (act == IO_COMPLETE) {
                rqp = recv_wrapper->rqp;
                send_wrapper = TAILQ_FIRST(&rqp->free_send);
                if (spdk_unlikely(send_wrapper == NULL)) {
        ...
        case RECON_START:
            ...
            if (is_q_while_need_q(recv_wrapper->cs_msg->type)) {
                bdev_context->buff = bdev_context->buff3;
            } else {
                bdev_context->buff = bdev_context->buff1;
            }
            bdev_read((void*) recv_wrapper);
        ...
        case RECON_IO_DONE:
            bdev_context->state = RELEASE;
            if (need_pq(recv_wrapper->cs_msg->type)) {
                if (is_q_while_need_q(recv_wrapper->cs_msg->type)) {
                    std::memset(bdev_context->buff1 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                0, recv_wrapper->cs_msg->fwd_length << bdev_context->blk_size_shift);
                } else if (is_p_while_need_q(recv_wrapper->cs_msg->type)) {
                    std::memset(bdev_context->buff3 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                0, recv_wrapper->cs_msg->fwd_length << bdev_context->blk_size_shift);
                }
            }
            if (is_data_while_need_q(recv_wrapper->cs_msg->type)) {
                gf_vect_mul(recv_wrapper->cs_msg->fwd_length << bdev_context->blk_size_shift,
                            gf_const_tbl_arr[recv_wrapper->cs_msg->data_index],
                            bdev_context->buff1 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                            bdev_context->buff3 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift));
            }
            ...
            if (rc == -ENOENT) {
                ...
                bdev_context->recv_cnt = recv_wrapper_in_map->bdev_context->recv_cnt;
                if (need_p(recv_wrapper->cs_msg->type)) {
                    raid5_xor_buf(bdev_context->buff1 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_wrapper_in_map->bdev_context->buff1 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_wrapper->cs_msg->fwd_length << bdev_context->blk_size_shift);
                }
                if (need_q(recv_wrapper->cs_msg->type)) {
                    raid5_xor_buf(bdev_context->buff3 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_wrapper_in_map->bdev_context->buff3 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                  recv_wrapper->cs_msg->fwd_length << bdev_context->blk_size_shift);
                }
                rdma_recv(recv_wrapper_in_map);
                if (bdev_context->recv_cnt == recv_wrapper->cs_msg->fwd_num) {
                    ...
                    if (recv_wrapper->cs_msg->next_index < 0) {
                        ...
                        TAILQ_REMOVE(&rqp->free_send, send_wrapper, link);
                        send_wrapper->ctx = recv_wrapper;
                        if (need_p_only(recv_wrapper->cs_msg->type)) {
                            map_ibv_send_wr_one2many(send_wrapper->send_wr_write, &recv_wrapper->cs_msg->sgl[recv_wrapper->cs_msg->num_sge[0]], recv_wrapper->cs_msg->num_sge[1],
                        ...
                        } else if (need_q_only(recv_wrapper->cs_msg->type)) {
                            map_ibv_send_wr_one2many(send_wrapper->send_wr_write, &recv_wrapper->cs_msg->sgl[recv_wrapper->cs_msg->num_sge[0]], recv_wrapper->cs_msg->num_sge[1],
                                                     (uint64_t) bdev_context->buff3 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                                     recv_wrapper->cs_msg->fwd_length << bdev_context->blk_size_shift, rqp, false);
                        } else {
                            map_ibv_send_wr_two2one(send_wrapper->send_wr_write, &recv_wrapper->cs_msg->sgl[recv_wrapper->cs_msg->num_sge[0]],
                                                    (uint64_t) bdev_context->buff1 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                                    recv_wrapper->cs_msg->fwd_length << bdev_context->blk_size_shift,
                                                    (uint64_t) bdev_context->buff3 + ((recv_wrapper->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                                    recv_wrapper->cs_msg->fwd_length << bdev_context->blk_size_shift, rqp, false);
        ...
        case RECON_DF_START:
            ...
            rqp = recv_wrapper->rqp;
            if (recv_wrapper->cs_msg->num_sge[0] > 0) {
                sig = recv_wrapper->cs_msg->num_sge[1] == 0;
                map_ibv_send_wr_one2many(recv_wrapper->send_wr_read, recv_wrapper->cs_msg->sgl, 1,
                                         (uint64_t) bdev_context->buff1 + ((bdev_context->offset % g_strip_size) << bdev_context->blk_size_shift),
                                         bdev_context->size << bdev_context->blk_size_shift, rqp, sig);
            }
            if (recv_wrapper->cs_msg->num_sge[1] > 0) {
                map_ibv_send_wr_one2many(&recv_wrapper->send_wr_read[recv_wrapper->cs_msg->num_sge[0]], &recv_wrapper->cs_msg->sgl[recv_wrapper->cs_msg->num_sge[0]], 1,
                                         (uint64_t) bdev_context->buff3 + ((bdev_context->offset % g_strip_size) << bdev_context->blk_size_shift),
                                         bdev_context->size << bdev_context->blk_size_shift, rqp, true);
            }
            rdma_read(recv_wrapper, recv_wrapper->cs_msg->num_sge[0] + recv_wrapper->cs_msg->num_sge[1]);
            break;
        case RECON_DF_READ_DONE:
            ...
            if (rc == -ENOENT) {
                ...
                if (recv_wrapper->cs_msg->num_sge[1] > 0) {
                    raid5_xor_buf(recv_wrapper_in_map->bdev_context->buff3 + ((bdev_context->offset % g_strip_size) << bdev_context->blk_size_shift),
                                  bdev_context->buff3 + ((bdev_context->offset % g_strip_size) << bdev_context->blk_size_shift),
                                  bdev_context->size << bdev_context->blk_size_shift);
                }
                rdma_recv(recv_wrapper);
                if (recv_wrapper_in_map->bdev_context->recv_cnt == recv_wrapper_in_map->cs_msg->fwd_num &&
                    ...
                    if (recv_wrapper_in_map->cs_msg->next_index < 0) {
                        ...
                        if (need_p_only(recv_wrapper_in_map->cs_msg->type)) {
                            map_ibv_send_wr_one2many(send_wrapper->send_wr_write, &recv_wrapper_in_map->cs_msg->sgl[recv_wrapper_in_map->cs_msg->num_sge[0]], recv_wrapper_in_map->cs_msg->num_sge[1],
                        ...
                        } else if (need_q_only(recv_wrapper_in_map->cs_msg->type)) {
                            map_ibv_send_wr_one2many(send_wrapper->send_wr_write, &recv_wrapper_in_map->cs_msg->sgl[recv_wrapper_in_map->cs_msg->num_sge[0]], recv_wrapper_in_map->cs_msg->num_sge[1],
                                                     (uint64_t) recv_wrapper_in_map->bdev_context->buff3 + ((recv_wrapper_in_map->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                                     recv_wrapper_in_map->cs_msg->fwd_length << bdev_context->blk_size_shift, rqp, false);
                        } else {
                            map_ibv_send_wr_two2one(send_wrapper->send_wr_write, &recv_wrapper_in_map->cs_msg->sgl[recv_wrapper_in_map->cs_msg->num_sge[0]],
                                                    (uint64_t) recv_wrapper_in_map->bdev_context->buff1 + ((recv_wrapper_in_map->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                                    recv_wrapper_in_map->cs_msg->fwd_length << bdev_context->blk_size_shift,
                                                    (uint64_t) recv_wrapper_in_map->bdev_context->buff3 + ((recv_wrapper_in_map->cs_msg->fwd_offset % g_strip_size) << bdev_context->blk_size_shift),
                                                    recv_wrapper_in_map->cs_msg->fwd_length << bdev_context->blk_size_shift, rqp, false);
                        }
...
req_handler_partial_write(struct recv_wr_wrapper_server *recv_wrapper)
{
    ...
    switch (cs_msg->type) {
        ...
        case FWD_MIX:
            ...
            break;
        case FWD_RO_PRO:
            bdev_context->state = FW_RO_PRO_START;
            left_bound = spdk_min(cs_msg->fwd_offset, cs_msg->y_offset);
            right_bound = spdk_max(cs_msg->fwd_offset + cs_msg->fwd_length, cs_msg->y_offset + cs_msg->y_length);
            bdev_context->offset = left_bound;
            bdev_context->size = right_bound - left_bound;
            break;
        case FWD_RW_PRO:
            bdev_context->state = FW_RW_PRO_START;
            left_bound = spdk_min(cs_msg->fwd_offset, cs_msg->y_offset);
            left_bound = spdk_min(left_bound, cs_msg->offset);
            right_bound = spdk_max(cs_msg->fwd_offset + cs_msg->fwd_length, cs_msg->y_offset + cs_msg->y_length);
            right_bound = spdk_max(right_bound, cs_msg->offset + cs_msg->length);
            bdev_context->offset = left_bound;
            bdev_context->size = right_bound - left_bound;
            break;
        default:
...
req_handler_parity(struct recv_wr_wrapper_server *recv_wrapper)
{
    ...
    switch(cs_msg->type) {
        ...
        case PR_MIX:
            ...
            break;
        case PR_PRO:
            bdev_context->state = PR_PRO_START;
            break;
        case PR_QRO:
            bdev_context->state = PR_QRO_START;
            break;
        default:
            SPDK_ERRLOG("Illegal message type: %d\n", cs_msg->type);
            assert(false);
...
connect_rdma_connections(void)
{
    ...
    for (i = 0; i < g_num_qp; i++) {
        ...
        for (j = 0; j < paraSend; j++) {
            ...
            cs_msg->req_id = (uint64_t) send_wrapper;
            for (k = 0; k < 16; k++) {
                struct ibv_sge *write_sge = (struct ibv_sge *) calloc(2, sizeof(struct ibv_sge));
                if (!write_sge) {
    ...
    for (i = 0; i < g_num_qp; i++) {
        ...
        qp_attr.cap.max_send_wr = spdk_max(paraSend * 17, 4096);
...
bdev_start(void *arg1)
{
    ...
    bdev_context->buff1 = (char*) spdk_dma_zmalloc(g_strip_size * bdev_context->blk_size, bdev_context->buf_align, NULL);
    if (!bdev_context->buff1) {
        spdk_put_io_channel(bdev_context->bdev_io_channel);
        spdk_bdev_close(bdev_context->bdev_desc);
        spdk_app_stop(-1);
    }
    bdev_context->buff2 = (char*) spdk_dma_zmalloc(g_strip_size * bdev_context->blk_size, bdev_context->buf_align, NULL);
    if (!bdev_context->buff2) {
        spdk_dma_free(bdev_context->buff1);
        spdk_put_io_channel(bdev_context->bdev_io_channel);
        spdk_bdev_close(bdev_context->bdev_desc);
        spdk_app_stop(-1);
    }
    bdev_context->buff3 = (char*) spdk_dma_zmalloc(g_strip_size * bdev_context->blk_size, bdev_context->buf_align, NULL);
    if (!bdev_context->buff3) {
        spdk_dma_free(bdev_context->buff1);
        spdk_dma_free(bdev_context->buff2);
        spdk_put_io_channel(bdev_context->bdev_io_channel);
        spdk_bdev_close(bdev_context->bdev_desc);
        spdk_app_stop(-1);
    }
    bdev_context->buff4 = (char*) spdk_dma_zmalloc(g_strip_size * bdev_context->blk_size, bdev_context->buf_align, NULL);
    if (!bdev_context->buff4) {
        spdk_dma_free(bdev_context->buff1);
        spdk_dma_free(bdev_context->buff2);
        spdk_dma_free(bdev_context->buff3);
        spdk_put_io_channel(bdev_context->bdev_io_channel);
        spdk_bdev_close(bdev_context->bdev_desc);
        spdk_app_stop(-1);
    }
    ...
    gf_const_g[0] = 1;
    for (int i = 1; i < 255; i++) {
        gf_const_g[i] = gf_mul(gf_const_g[i-1], 2);
    }

    for (int i = 0; i < 255; i++) {
        gf_vect_mul_init(gf_const_g[i], gf_const_tbl_arr[i]);
    }

    unsigned char u,d;
    for (int i = 0; i < 255; i++) {
        d = gf_inv(gf_const_g[i] ^ 1);
        for (int j = 0; j < 255; j++) {
            u = gf_const_g[j] ^ 1;
            gf_vect_mul_init(gf_mul(u, d), gf_const_tbl_arr_p[i][j]);
            u = gf_const_g[j] ^ gf_const_g[i];
            gf_vect_mul_init(gf_mul(u, d), gf_const_tbl_arr_q[i][j]);
            u = gf_const_g[j];
            gf_vect_mul_init(gf_mul(u, d), gf_const_tbl_arr_b[i][j]);
        }
        u = gf_const_g[i];
        gf_vect_mul_init(gf_mul(u, d), gf_const_tbl_arr_a[i]);
    }
